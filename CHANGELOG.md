...
- **Grafiki SVG** (`docs/schematy/`, generator `generate_svgs.py`): pełny **schemat elektryczny** wszystkich połączeń (piny z `src/config.h`, złącza J1–J5, zawory, WiFi do modułu 7", przyciski soft-key na MCP23017), makieta ekranu roboczego oraz **4 propozycje wizualizacji panelu** (A kabinowy, B pas pod ekranem, C pionowy z klawiszami fizycznymi na wysokości etykiet wzorców, D grzybek STOP) — opis w `docs/WIZUALIZACJE.md`.
